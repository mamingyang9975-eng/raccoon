...
export async function onRequestPost(context) {
  try {
    ...
    const model = body?.model || env.OPENROUTER_MODEL || "meta-llama/llama-3.3-8b-instruct:free";
    ...
    if (!res.ok) {
      ...
      return json({ error: `openrouter error: ${res.status}`, detail: t.slice(0, 200) }, 502);
    }
